//
//  ExecutionMatrixDefinitionTests.swift
//  QALenz
//
...
// 실행 행렬 dimension 계약이 입력 순서와 정책 값을 보존하는지 검증합니다.
@Suite
struct ExecutionMatrixDefinitionTests {
	...
	func dimension_계약이_입력_순서와_기본값을_보존한다() {
		...
		let defaults = ExecutionMatrixDefaults(
			devices: ["iPhone 16"],
	...
	func 정책이_지원_쌍과_target_상한을_보존한다() {
		let supportedPair = ExecutionMatrixDeviceOperatingSystem(
			device: "iPhone 17",
			operatingSystem: "iOS 26.0"
		)
		let policy = ExecutionMatrixPolicy(
