//
//  ExecutionMatrixValidatorTests.swift
//  QALenz
//
...
// 실행 행렬 입력 해석과 의미 검증 계약을 검증합니다.
@Suite
struct ExecutionMatrixValidatorTests {
	// 명시한 dimension과 누락 dimension의 기본값을 함께 정규화하는지 검증합니다.
	@Test
	func 명시한_dimension과_기본값을_정규화한다() throws {
		let definition = try ExecutionMatrixDefinitionDecoder().decode(.object([
	...
	func 지원하지_않는_dimension_key를_거부한다() {
		#expect(throws: ExecutionMatrixValidationError.unsupportedDimension("device")) {
			try ExecutionMatrixDefinitionDecoder().decode(.object([
	...
	func 잘못된_dimension_형식과_빈_배열을_거부한다() {
		#expect(throws: ExecutionMatrixValidationError.dimensionInvalid(.languages)) {
			try ExecutionMatrixDefinitionDecoder().decode(.object([
				"languages": .string("ko")
			]))
		}
		#expect(throws: ExecutionMatrixValidationError.dimensionEmpty(.appearances)) {
			try ExecutionMatrixDefinitionDecoder().decode(.object([
				"appearances": .array([])
			]))
		}
	...
	func 빈_dimension_값과_지원하지_않는_device_OS_조합을_거부한다() {
		#expect(throws: ExecutionMatrixValidationError.valueEmpty(.devices, 0)) {
			try ExecutionMatrixValidator().validate(
				.init(devices: [" "]),
				defaults: defaults(),
				policy: policy()
			)
		}
		#expect(throws: ExecutionMatrixValidationError.deviceOperatingSystemUnsupported(
			device: "iPhone 17",
	...
	private func policy() -> ExecutionMatrixPolicy {
		.init(
