...
struct OutputDefinition: Sendable {
	let versions: Set<String>
	let payload: PayloadDefinition
	let result: ResultRule
...
enum ResultRule: Sendable {
	case passed
	case summaryStatus

	// 판정 규칙에 따라 payload를 공통 실행 결과로 변환합니다.
	func normalizedResult(from payload: XcodeBuildMCPPayload?) throws -> RunResult {
		switch self {
		case .passed:
			return .passed
		case .summaryStatus:
...
struct PayloadDefinition: Sendable {
	let isRequired: Bool
	let schema: PayloadSchema

	// raw payload를 허용된 구조로 투영하고 구조가 다르면 거부합니다.
	func projected(_ payload: XcodeBuildMCPPayload?) throws -> XcodeBuildMCPPayload? {
...
indirect enum PayloadSchema: Sendable {
	case scalar
...
private enum PayloadDefinitionError: Error {
	case invalid
}
